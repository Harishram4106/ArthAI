...
router.post('/generate', auth_routes_1.requireAuth, async (req, res) => {
    try {
        ...
        const products = await server_1.prisma.productFact.findMany();
        const allocations = computeAllocation(surplus, assessment.profile, products);
        const portfolio = await server_1.prisma.portfolioRecommendation.create({
            data: {
                userId,
                items: {
...
router.get('/current', auth_routes_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
...
router.post('/consent', auth_routes_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { portfolioId } = req.body;
        await server_1.prisma.portfolioRecommendation.update({
            where: { id: portfolioId },
            data: { status: 'Consented' }
        });
        await server_1.prisma.consentRecord.create({
            data: {
                userId,
                policyVersion: 'v1.0',
