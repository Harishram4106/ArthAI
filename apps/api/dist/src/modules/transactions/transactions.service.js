...
class TransactionsService {
    ...
    async getFinancialSnapshot(userId) {
        ...
        if (discretionaryRatio > 0.4)
            expenseStabilityScore = 40;
        else if (discretionaryRatio > 0.3)
            expenseStabilityScore = 60;
        else if (discretionaryRatio > 0.2)
            expenseStabilityScore = 80;
        ...
        const healthScore = Math.round((savingsRateScore * 0.35) +
     ...
     */
    async ingestCsv(userId, csvData) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (rLower.includes('salary') || rLower.includes('techcorp'))
                category = 'Income';
            else if (rLower.includes('zomato') || rLower.includes('swiggy'))
                category = 'Dining';
            else if (rLower.includes('bigbasket') || rLower.includes('dmart'))
                category = 'Groceries';
            else if (rLower.includes('rent'))
                category = 'Rent';
            else if (rLower.includes('electricity') || rLower.includes('bill'))
                category = 'Bills';
