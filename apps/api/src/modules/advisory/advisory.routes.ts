...
function generateDeterministicResponse(message: string, context: any) {
  ...
  if (lowerMsg.includes('health') || lowerMsg.includes('score')) {
    ...
    if (breakdown) {
      let insights = [];
      if (breakdown.savingsRate.score < 80) insights.push(`Your savings rate (${breakdown.savingsRate.value}) needs work.`);
      if (breakdown.emergencyFund.score < 70) insights.push(`Your emergency fund (${breakdown.emergencyFund.value}) should be topped up.`);
      if (breakdown.expenseStability.score < 80) insights.push(`Your discretionary expenses are a bit high (${breakdown.expenseStability.value}).`);
      if (breakdown.goalReadiness.score < 80) insights.push(`Some of your goals are at risk.`);
  ...
  try {
    ...
    if (aiResponse.needs_human_advisor || aiResponse.is_product_recommendation) {
      await createAuditLog(
        userId,
        aiResponse.needs_human_advisor ? 'Human Escalation Triggered' : 'AI Product Recommendation Provided',
        'Advice',
        {
          threadId: activeThreadId,
          messageId: botMsg.id,
